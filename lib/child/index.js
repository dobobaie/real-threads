...
module.exports = class $cthread
{
  ...
  connect(url, guuid) {
    ...
    socket.on('hello', () => {
      ...
      socket.emit('hello', { guuid })
      this.modules.event.emit(`${guuid}_client_connected`, thread);
    });
    socket.on('request', ({ request }) => {
      this.debug('[DEBUG]', `CTHREAD-PARENT: socket received request`, request);
      this.modules.event.emit(`${guuid}_client_request`, request);
    });
    ...
    return new Promise(resolve =>
      thread.on(`${guuid}_client_connected`, resolve)
    );
  }
};
